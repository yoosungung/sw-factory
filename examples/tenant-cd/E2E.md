...
5. pm: 아래 네 증거 그룹이 모두 있을 때만 Done.
...
| 증상 | 점검 |
|------|------|
| ta가 registry에 테넌트 없음 | render + ta Pod 재시작, `tenant_cd.enabled: true` |
| workflow_dispatch 실패 | 테넌트 repo Actions 권한, workflow 파일 경로=`tenant_cd.workflow` |
| smoke 실패 | `verify.smoke.url`이 ta Pod에서 라우팅 가능한지, Service/포트 |
| pm가 Done 거부 | 증거 필드 4그룹 누락 — ARCHITECTURE §2.8 |
