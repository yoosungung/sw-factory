...
- **Non-modal Inspector (기본):**
  ...
  - 패널 상단에 `[전체 화면으로 열기 ↗]` 및 `[닫기 ✕]` 제공.
- **Centered Modal (집중 모드):**
  - 단축키 또는 명시적 모달 모드(`?issueUi=modal`) 호출 시에만 어두운 배경 딤과 함께 화면 정중앙에 팝업.
- **폼 컨트롤:**
  - 투박한 브라우저 기본 `<select>` 대신, 컬러 뱃지와 아이콘이 들어간 커스텀 상태/우선순위 팝오버 버튼 적용.
