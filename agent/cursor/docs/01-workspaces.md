...
세션 쿠키·`mcp.json`은 [factory-mcp seed](../mcp/seed.ts)가 생성(번들 mcp.json은 사용하지 않음). stdio 진입점은 **절대 경로**(`AGENT_APP_ROOT`, Docker=`/app`). entrypoint의 apply-persona-seeds가 기존 쿠키 workspace의 mcp.json을 refresh한다.  
