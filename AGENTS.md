...
M0–M5: `leantime-plugin/`, `agent-runner/`, `deploy/k8s/`, Tenant CD(`repos[].tenant_cd`·ta `tenant-cd`). M4: PVC/spend CronJobs. **seewin** Phase A+B + Hermes cron 컷오버. **M10** org-wiki. **M11** Dual-loop: 직원 5인(PM/KM/QA/TA/AA), tenant≡`client_id`, 기능(test→QA∥AA→prod) + 주간 NF(부하·클린·대량품질). CI: `.github/workflows/ci.yml`. 제품 소스·품질 기준은 테넌트 repo. `settings.schedules` → `cursorbridge-schedule-tick`. CursorBridge My Work **Created by me** 위젯(기본 Calendar 슬롯 대체; author + open/Done≤5d).
