...
- 범위: `In Progress` · `Review` · `Deploying*` · `QA`. `Blocked`/`New`/`Done`/`Archived` timebox 금지. Approval은 misroute sweep + **storm 터미널**.
- Silence reset = assignee 실진행 / `nf-progress:` / 완료·blocker만 (ladder `@mention`·status-board·mention-outcome은 reset 아님).
...
`@eric`(admin `type: human`, id는 bridge — 이름 하드코딩 금지) 대상: 요구 충돌, 범위 확장, 비용·계약·다운타임, AC 제품 판단, 보안 정책 모호, **권한/시크릿**, stall 터미널, **mention/comment storm**.  
Human-only: Approval + admin assignee + 구체 ask — `Blocked`로 두지 않음(명확 FS deps만 Blocked+마커). 라우팅은 **다음 실행 가능자** 기준; pm은 elevated executor가 아님. 스톰 ask에는 루프 증거(멘션 횟수·결측 게이트)·중단할 re-mention을 적는다.
