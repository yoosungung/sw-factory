...
2. **SLA by lane:**
   ...
   - `Deploying Test` / `Deploying Prod` / `QA`: stall if last actionable specialist comment/evidence is older than **2 hours** and there is no newer `@mention`/progress. **`nf-progress:` (and equivalent progress heartbeats) count as evidence and reset the stall clock** (ARCHITECTURE §2.6 #10/#14). **Ladder:** (1) first action = re-`@mention` the **current assignee** (ta / qa / aa / km) with a **health check only**; (2) if that health-check is ≥**1 hour** old with still no assignee progress/`nf-progress:`, `@ta` with marker **`assignee-runtime-check`** (Pod Ready + runner logs for that ticket: `session.prompt.skipped`/`session.recover`/zombie `active_run`) — do **not** ask ta to run EX/E2E/security/CD; (3) after TA outcome: alive → re-mention original assignee to resume; dead → restart/`Blocked`/new session (skip duplicate restart if recent `session.recover` already succeeded — R1–R5). **Do not** re-run the long job yourself; do not run kubectl, tenant_cd, browser E2E, or security review yourself. Do not escalate this path to Eric unless human-only privilege is proven.
...
4. **Status-board upsert (anti-spam):** For each flow-active candidate, find a pm-authored comment containing `<!-- pm-checkpoint-status -->`. If found, `edit_comment` that id with the latest Acted/Skipped/SLA summary (**no** HTML `@mention` anchors in the board). If missing and you need a durable skip/SLA record, `add_comment` **once** with the marker as the first line. Never `add_comment` new `PM verify` / `Outcome record only` / `no rewrite` spam. `edit_comment` counts as a Leantime write for success_checks.
5. Identify the last actionable owner comment. If past the lane SLA and there is no PR/test/deploy/QA/completion/blocker evidence after it, **`add_comment` a new actionable handoff** (not an edit of the status board):
   - `In Progress`: checkpoint request (attempted work, single cause, branch/PR, next minimum step) with developer `@mention`.
   - Deploy/QA: re-handoff `@mention` + required evidence only; or `@ta` `assignee-runtime-check` when the post-health-check 1h grace elapsed.
   - Suppress duplicate **actionable** handoffs of the same kind within 30 minutes on the same ticket.
