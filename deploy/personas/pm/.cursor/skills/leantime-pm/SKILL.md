...
2. **SLA by lane:**
   ...
   - `Deploying Test` / `Deploying Prod` / `QA`: stall if last actionable specialist comment/evidence is older than **2 hours** and there is no newer `@mention`/progress. **`nf-progress:` (and equivalent progress heartbeats) count as evidence and reset the stall clock** (ARCHITECTURE §2.6 #10/#14). Re-`@mention` the assignee (ta / qa / aa / km as appropriate) with a **health check only**: alive → ask for next progress; dead/no heartbeat → restart or `Blocked`. **Do not** re-run the long job yourself; do not run kubectl, tenant_cd, browser E2E, or security review yourself.
