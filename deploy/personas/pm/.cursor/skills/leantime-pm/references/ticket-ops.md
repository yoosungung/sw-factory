...
7. **Flow stall (ARCHITECTURE §2.6 #14):** `pm-checkpoint` also watches `Review` / `Deploying Test` / `QA` / `Deploying Prod`. Deploy/QA stalls (≥2h without evidence) get a re-`@mention` only — PM does not execute TA/QA/AA work. Treat `nf-progress:` / progress heartbeats as evidence (stall clock reset). On nudge: verify alive vs dead; do not relaunch a still-running long NF job.
