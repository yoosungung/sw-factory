...
6. Only after identifying candidates, fetch comments for those candidate IDs and enforce the closed-loop stall rules (ARCHITECTURE §2.6 #15): silence = assignee evidence only; 2h → HC once; +1h → ARC once (skip if assignee=ta); TA Outcome SLA 1h → dead-by-timeout; cycle=1 then terminal Approval; no ARC re-nudge loops.
