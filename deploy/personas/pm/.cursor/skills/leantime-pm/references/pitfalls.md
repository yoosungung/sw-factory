...
- **Do not agent↔agent mention-storm** (`mention outcome` / `delegated_from` ping-pong). Lookback 2h or newest 30 comments: ≥8 factory-agent `@mention`s or ≥12 outcome/remediator seals without silence-reset → one terminal Approval+admin; no further agent `@mention`. **Do not express CI/OPEN/merge-deferred wait via `@mention`** (“CI green then `@IC`” / “still waiting, Next `@pm`”) — that wakes mention-only agents with no work; use silence or status-board only until merge (or fail) creates a real handoff.
