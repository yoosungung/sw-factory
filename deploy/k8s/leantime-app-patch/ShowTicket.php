...
class ShowTicket extends Controller
{
    ...
    public function get($params): Response
    {
        ...
        // Factory overlay: Discussion pages large threads (e.g. 800+) — initial window only.
        $commentLimit = 20;
        $commentOffset = 0;
        $numComments = $this->commentService->countComments('ticket', $id);
        $page = $this->commentService->getComments('ticket', $id, 0, 0, $commentLimit + 1, $commentOffset);
        $page = is_array($page) ? $page : [];
        $commentsHasMore = count($page) > $commentLimit;
        $comments = $commentsHasMore ? array_slice($page, 0, $commentLimit) : $page;

        $this->tpl->assign('numComments', $numComments);
        $this->tpl->assign('comments', $comments);
        $this->tpl->assign('commentLimit', $commentLimit);
        $this->tpl->assign('commentOffset', $commentOffset);
        $this->tpl->assign('commentsHasMore', $commentsHasMore);
        $this->tpl->assign('commentModule', 'ticket');
        $this->tpl->assign('commentModuleId', $id);
