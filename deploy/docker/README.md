...
`AGENT_APP_ROOT=/app` — factory-mcp stdio 절대경로. 기동 시 persona별 로그인 후 mcp.json을 쓰고, 기존 쿠키 workspace는 refresh.
...
| `entrypoint.sh` | apply persona seeds → persona 쿠키 로그인 → ensure-repos → cursor listen → gateway poll |
