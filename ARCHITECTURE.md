...
13. **티켓 간 FS 선행 (blocked-by)** — SoR은 description HTML 마커 `<!-- blocked-by:ID[,ID] -->` + 선행 미완료 시 `Blocked`. Leantime `dependingTicketId`는 **parent/subtask 계층만**(blocked-by로 쓰지 않음). human/pm이 선행을 지시·발견하면 **같은 세션**에서 보드에 등록·`get_ticket` 검증(코멘트 ACK만으로 대체 금지). 선행 Done 후 마커 clear·레인 bounce.
14. **human 오배정/오멘션 정정 (PM)** — 주기 점검(`pm-checkpoint`) 때 `Waiting for Approval`·`@eric` 요청을 훑는다. 다음 액션이 에이전트 실행 가능(PR 리뷰/머지·QA E2E·AA·TA CD·KM wiki·구현)이면 올바른 상태·assignee·`@mention`으로 되돌리고 정정 코멘트를 남긴다. 시크릿·RBAC·제품/범위 판단 등 사람 전용 ask는 Approval 유지(모호하면 유지).
15. **기능 루프 진행 관리 (PM)** — `pm-checkpoint`(`gates: [flow_active]`)는 `In Progress`뿐 아니라 `Review`·`Deploying Test`·`QA`·`Deploying Prod` 정체도 본다. 개발 timebox(≈30분)와 Deploy/QA stall(≈2h)을 구분한다. Deploy/QA에서 PM은 **실행하지 않는다**(TA CD·QA E2E·AA 보안은 각 레인). PM은 kubectl 금지. soft `budget.timeout_ms` 만료 ≠ 복구 완료. R1–R5는 runner 전용(별계층).
    - **Silence / 시계 reset:** assignee **실진행**·`nf-progress:`(또는 동등)·완료/blocker 증거만 stall 시계를 리셋한다. PM/TA ladder `@mention`·status-board·remediator seal은 reset **금지**.
    - **Deploy/QA stall 사다리(closed-loop):** (1) silence ≥2h → 담당 assignee `@mention` health-check **1회**; (2) HC 이후 ≈1h 무 assignee 증거 → assignee가 **ta가 아니면** `@ta` **assignee-runtime-check** **사이클당 1회**(Pod Ready·runner 로그 `session.prompt.skipped`/`session.recover`·zombie/`active_run` — EX/E2E/보안/CD 대행 금지). **ARC re-nudge 금지.** assignee=`ta`이면 ARC self-check를 **생략**하고 바로 dead-by-timeout 경로; (2→3) TA Outcome SLA: 첫 ARC 후 **≥1h** 무 Outcome(`Verdict: alive|dead`) → PM이 **dead-by-timeout**으로 간주(kubectl 없이); (3) alive → 원 assignee resume; dead / dead-by-timeout → restart/`Blocked`/새 session(최근 `session.recover` 성공이면 재기동 중복 금지·원 assignee 재부착). **`@mention`만으로 alive 금지.**
    - **Cycle cap = 1:** HC→(ARC|timeout)→resume/restart **1사이클** 후에도 assignee 증거 없으면 **터미널**. assignee=`ta`이거나 restart 실행자가 없으면 dead-by-timeout 직후 즉시 터미널.
    - **터미널:** `Waiting for Approval` + admin human(`agents[]` `type: human`, bridge `leantime_user_id` — 이름 하드코딩 금지) `@mention`과 구체 ask(세션 재기동·TA 응답·권한). #14: 이 ask가 admin 전용이면 Approval Keep.
