...
| **A1** | Worker `agent_event_log` + `GET /api/agent/events` | 마이그레이션·mutate append·pull API 테스트 green | planned |
| **A2** | agent/gateway 구현 (tail·라우팅·localhost dispatch·retry/`acked_id`) | mock cursor에 이벤트→202 배달 E2E | planned |
