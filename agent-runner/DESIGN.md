...
| `session.recover` | ticket 매핑 forget + cancel/`force` + 재create (R1–R3) |
...
| R5 | 단위 테스트: accepted 후 child `exit` → worker idle 복귀·매핑 없음·다음 create가 새 agent_id | ✅ `worker-pool.test` · `session-manager-recovery.test` |
